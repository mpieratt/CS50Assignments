...
function AlienLaunchMarker:update(dt)
    ...
    --Spawn Triplets if mouse 2 was pressed 
    if love.mouse.wasPressed(2) then 
        local alienX, alienY = self.alien.body:getPosition()
        local alienXVel, alienYVel = self.alien.body:getLinearVelocity()
        self.alienDuex = Alien(self.world, 'round', alienX, alienY, 'Player')
        self.alienDuex.body:setLinearVelocity(alienXVel, alienYVel*1.2 )

        
        self.alienTri = Alien(self.world, 'round', alienX, alienY, 'Player')
        self.alienTri.body:setLinearVelocity(alienXVel, alienYVel*0.8 )
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        if self.alienDuex then 
            self.alienDuex:render()
        end

        if self.alienTri then 
            self.alienTri:render()
        end
    end
end